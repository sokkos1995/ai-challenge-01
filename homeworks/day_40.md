...
| 3 | local `qwen2.5-coder:7b` | **3/18** | `fizzbuzz` | нерабочий CLI: `main(['5'])` ждёт `argv` как `sys.argv` (нужен script name) | ~0.27 мин | **50%** (9/18) | **5** |
...
Сравнение cloud vs local: streak **18 → 3**; минут **2–3 vs 5**; first-try **100% → 50%**. Локальная 7B тянет простые bugfix, ломается на контракте CLI/`argv` и части refactor/test/docs. Харнесс: `homeworks/src/day_40_run_local_loop.py`, лог: `homeworks/artifacts/day_40_local/`.
...
- [x] Сравнение с локальной моделью — streak **3/18**, **5 минут**, first-try 50%

Формат сдачи: **количество времени без пауз (в минутах)** по прогонам.

Прогон 1 (факт): **3 минуты**.  
Прогон 2 (факт): **2 минуты**.  
Прогон 3 local (факт): **5 минут** (подряд без срыва критерия Done: до задачи 3 включительно ≈ **1 минута** чистого streak-окна; полный прогон пула 5 мин).
