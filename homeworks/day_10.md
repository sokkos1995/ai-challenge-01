...
## Демо: один сценарий на 3 стратегиях

Сценарий (10 сообщений): он специально содержит “липкие” факты (стек, дедлайн, предпочтение по формату), чтобы было видно, что агент их не теряет.
...
Для каждой стратегии (`sliding`, `facts`, `branching`) прогоните сценарий и сравните:

- качество ответа (насколько корректно восстановлены “липкие” факты);
- стабильность (не теряются ли ключевые детали при росте истории);
- расход токенов (по `prompt_tokens_total`, `dialog_history_tokens`, `total_tokens`);
- удобство (как быстро пользователь “доходит” до нужной ветки/ответа в branching).

### Автопрогон (пример кода)

Вставьте в терминал (скрипт сам прогонит `sliding` и `facts`, а для `branching` покажет 2 независимые ветки от checkpoint):
...
Смысл: вы получите метрики по токенам и увидите последние `agent>`-ответы. Качество всё равно оценивайте по тому, насколько корректно агент сохранил стек/дедлайн и попал в желаемый формат.
...
Удобно запускать в интерактивном режиме:

- Sliding Window:
  - `python3 llm_cli.py --chat --context-strategy sliding`
- Sticky Facts (key-value):
  - `python3 llm_cli.py --chat --context-strategy facts`
- Branching (2 ветки):
  - `python3 llm_cli.py --chat --context-strategy branching`

Для `sliding` и `facts` размер окна берется из `LLM_CHAT_KEEP_LAST_N` (по умолчанию `10`).

### Команды внутри `branching`

- `@checkpoint` — сохранить checkpoint (точку разветвления)
- `@fork` — создать 2 ветки (`1` и `2`) от checkpoint и активировать ветку `1`
- `@switch 1` / `@switch 2` — переключиться на ветку
- `@branches` или `@branch-info` — показать текущее состояние веток