...
Когда пользователь просит **закоммитить** / **запушить** изменения — полный флоу описан в разделе **Branch / commit / push**: всегда `main` → `git pull` → новая ветка от `main` → коммит → `pre-push-secrets` → push (не коммитить в `main`; не ветвиться от предыдущего ДЗ; PR создаёт пользователь на GitHub).
...
### Branch / commit / push (когда пользователь просит закоммитить / запушить)

Не коммитить напрямую в `main`. **Каждое новое ДЗ / фича — всегда новая ветка от актуального `main`**, не от предыдущей homework-ветки.

Обязательный алгоритм:

1. **Сохранить незакоммиченные правки** (если есть незакоммиченный worktree и нужно сменить ветку): `git stash push -u -m …` или убедиться, что нужные файлы уже в индексе/патче.
2. **`git checkout main`** (или `master`).
3. **`git pull`** (подтянуть remote `main`).
4. **Создать новую feature-ветку от `main`:** `git checkout -b <имя>`  
   Имя — короткое по смыслу (`hw-43-model-routing`, `fix-storage-json`).  
   **Запрещено:** делать `git checkout -b …` от другой homework-ветки и наслаивать ДЗ друг на друга.
5. **Коммит:** закоммитить только релевантные файлы этого задания (формат сообщения — выше). Не включать секреты (`.env`, токены) и gitignored-артефакты.
6. **Push:** `pre-push-secrets` → при вердикте `OK` → `git push -u origin HEAD`.
7. **PR:** агент **не** создаёт Pull Request сам. Пользователь открывает PR на GitHub вручную.

Если нужная ветка уже существует на remote, но была ошибочно создана не от `main` — пересоздать от `main` (cherry-pick / новый коммит только нужных файлов) и обновить remote (`--force-with-lease` только для этой feature-ветки, никогда для `main`/`master`).

После push сообщить пользователю имя ветки и что можно открывать PR.

**Антипаттерн:** `checkout` предыдущего `hw-N-…` → `checkout -b hw-(N+1)-…` → push. Так в PR попадает чужое ДЗ.
...
- **Скилл `post-pr-qa`** — после PR / после деплоя фичи: Level-1 pytest + Level-2 smoke → `qa_report` (субагент `post-pr-qa`).
- **Субагент `python-reviewer`** — ревью диффа на соответствие AGENTS.md.
- **Субагент `test-runner`** — изолированный прогон/починка тестов.
- **Субагент `post-pr-qa`** — единый QA-flow day_38 (код + UI smoke); режим B — обновить smoke под фичу и прогнать всё.
- **Субагент `codebase-explorer`** — широкий поиск по репо перед крупным рефакторингом.
- **Субагент `bug-fix`** — баг: причина, минимальный фикс, регрессия-тесты (rule: `.cursor/rules/bug-fix.mdc`).
- **Субагент `research`** — вопрос по кодовой базе, readonly, структурированный ответ (rule: `.cursor/rules/research.mdc`).
