...
Когда пользователь просит **закоммитить** изменения — полный флоу ветка → коммит → push описан в разделе **Branch / commit / push** (не коммитить в `main`; PR создаёт пользователь на GitHub).
...
### Branch / commit / push (когда пользователь просит закоммитить)

Не коммитить напрямую в `main`. Флоу:

1. **Ветка:** если текущая ветка — `main` (или `master`), создать новую feature-ветку и переключиться на неё (`git checkout -b …`). Имя ветки — короткое и по смыслу изменений (например `hw-38-smoke-qa`, `fix-storage-json`). Если уже не на `main` — коммитить в текущую feature-ветку.
2. **Коммит:** закоммитить все релевантные изменения в эту ветку (формат сообщения — выше). Не включать секреты (`.env`, токены) и артефакты, которые не должны попадать в git.
3. **Push:** после успешного коммита — `pre-push-secrets`, затем `git push -u origin HEAD` (отправить ветку на GitHub).
4. **PR:** агент **не** создаёт Pull Request сам. Пользователь открывает PR на GitHub вручную, чтобы отработали CI/проверки.
